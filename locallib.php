...
/**
 * Theme Boost isf_espanhol - Locallib file
 *
...
//  * @return string
//  */
// function theme_boost_isf_espanhol_get_random_loginbackgroundimage_class() {
//
//     // Fetch context.
...
//     $files = $fs->get_area_files($systemcontext->id, 'theme_boost_isf_espanhol', 'loginbackgroundimage', false, 'itemid', false);
//
...
//  * @return string
//  */
// function theme_boost_isf_espanhol_get_loginbackgroundimage_scss() {
//     $count = 0;
//     $scss = "";
//
//     // Fetch context.
...
//  * @return array
//  */
// function theme_boost_isf_espanhol_get_imageareacontent() {
//     // Get cache.
//     $themeboostisf_espanholcache = cache::make('theme_boost_isf_espanhol', 'imagearea');
//     // If cache is filled, return the cache.
//     $cachecontent = $themeboostisf_espanholcache->get('imageareadata');
...
//         $fs = get_file_storage();
//         // Get all files from filearea.
//         $files = $fs->get_area_files($systemcontext->id, 'theme_boost_isf_espanhol', 'imageareaitems', false, 'itemid', false);
//
...
//             $lines = explode("\n", get_config('theme_boost_isf_espanhol', 'imageareaitemslink'));
//             // Parse item settings.
...
//                     // Fill the cache.
//                     $themeboostisf_espanholcache->set('imageareadata', $imageareacache);
...
//             return $themeboostisf_espanholcache->set('imageareadata', array());
//         }
//     }
// }
 ...
 */
function theme_boost_isf_espanhol_process_flatnav(flat_navigation $flatnav) {
    global $USER;
    // If the setting defaulthomepageontop is enabled.
    if (get_config('theme_boost_isf_espanhol', 'defaulthomepageontop') == 'yes') {
        ...
        if (($coursehomenode = $flatnav->find('coursehome', global_navigation::TYPE_CUSTOM)) != false) {
            ...
            if (get_config('core', 'defaulthomepage') == HOMEPAGE_SITE) {
                ...
                $flatnavreturn = theme_boost_isf_espanhol_set_node_on_top($flatnav, 'home', $coursehomenode);
            } else if (get_config('core', 'defaulthomepage') == HOMEPAGE_MY) { // If the dashboard is set as the default homepage
                // by the admin.
                // Return the modified flat_navigtation.
                $flatnavreturn = theme_boost_isf_espanhol_set_node_on_top($flatnav, 'myhome', $coursehomenode);
 ...
 * @return flat_navigation.
 */
function theme_boost_isf_espanhol_set_node_on_top(flat_navigation $flatnav, $nodename, $beforenode) {
 ...
 */
function theme_boost_isf_espanhol_get_incourse_settings() {
    global $COURSE, $PAGE;
    // Initialize the node with false to prevent problems on pages that do not have a courseadmin node.
    $node = false;
    // If setting showsettingsincourse is enabled.
    if (get_config('theme_boost_isf_espanhol', 'showsettingsincourse') == 'yes') {
        ...
        if ($PAGE->context->contextlevel == CONTEXT_COURSE || $PAGE->context->contextlevel == CONTEXT_MODULE) {
            ...
            if (!empty($node)) {
                ...
                if (get_config('theme_boost_isf_espanhol', 'incoursesettingsswitchtorole') == 'yes' && !is_role_switched($COURSE->id)) {
                    ...
                    if (is_array($roles) && (count($roles) > 0)) {
                        ...
                        $properties = array('text' => get_string('switchroleto', 'theme_boost_isf_espanhol'),
 ...
 * @return navigation_node.
 */
function theme_boost_isf_espanhol_get_incourse_activity_settings() {
    global $PAGE;
