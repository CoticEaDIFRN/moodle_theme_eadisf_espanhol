...
/**
 * Theme Boost isf_espanhol - Library
 *
 ...
 * @return string
 */
function theme_boost_isf_espanhol_get_main_scss_content($theme) {
    global $CFG;
    ...
    } else if ($filename == 'isf_espanhol.scss') {
        // We still load the default preset files directly from the boost theme. No sense in duplicating them.
        $scss .= file_get_contents($CFG->dirroot . '/theme/boost_isf_espanhol/scss/preset/isf_espanhol.scss');

    } else if ($filename == 'isf_espanhol_presencial.scss') {
        // We still load the default preset files directly from the boost theme. No sense in duplicating them.
        $scss .= file_get_contents($CFG->dirroot . '/theme/boost_isf_espanhol/scss/preset/isf_espanhol_presencial.scss');

    } else if ($filename && ($presetfile = $fs->get_file($context->id, 'theme_boost_isf_espanhol', 'preset', 0, '/', $filename))) {
        // This preset file was fetched from the file area for theme_boost_isf_espanhol and not theme_boost (see the line above).
    ...
    // $pre = file_get_contents($CFG->dirroot . '/theme/boost_isf_espanhol/scss/pre.scss');
    // Post CSS - this is loaded AFTER the main scss but before the extra scss from the setting.
    // $post = file_get_contents($CFG->dirroot . '/theme/boost_isf_espanhol/scss/post.scss');
 ...
 * @return array
 */
function theme_boost_isf_espanhol_get_pre_scss($theme) {
    global $CFG;
    // MODIFICATION START.
    require_once($CFG->dirroot . '/theme/boost_isf_espanhol/locallib.php');
    // MODIFICATION END.
    ...
    // $scss .= theme_boost_isf_espanhol_get_loginbackgroundimage_scss();
    // MODIFICATION END.
 ...
 * @return bool
 */
function theme_boost_isf_espanhol_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = array()) {
    if ($context->contextlevel == CONTEXT_SYSTEM) {
        $theme = theme_config::load('boost_isf_espanhol');
        if ($filearea === 'favicon') {
...
/**
 * If setting is updated, use this callback to clear the theme_boost_isf_espanhol' own application cache.
 */
function theme_boost_isf_espanhol_reset_app_cache() {
    // Get the cache from area.
    $themeboostisf_espanholcache = cache::make('theme_boost_isf_espanhol', 'imagearea');
    // Delete the cache for the imagearea.
    $themeboostisf_espanholcache->delete('imageareadata');
...
function get_isf_espanhol_commom_moodle_template_context()
{
...
function get_isf_espanhol_calendario() {
    global $CFG, $COURSE;
...
function get_isf_espanhol_course_content_actions()
{
    global $PAGE, $COURSE;
...
function get_isf_espanhol_course_common_actions()
{
    global $PAGE, $COURSE;
    if ($PAGE->pagelayout == "course" || $PAGE->pagelayout == "incourse") {
        $extraflatnav = [];
...
function get_isf_espanhol_template_context()
{
    global $PAGE;

    $templatecontext = get_isf_espanhol_commom_moodle_template_context();

    if ($templatecontext['in_course_page'] || $templatecontext['within_course_page']) {
        $templatecontext['course_content_actions'] = get_isf_espanhol_course_content_actions();
        $templatecontext['course_common_actions'] = get_isf_espanhol_course_common_actions();
    }
    $templatecontext['nosso_calendario'] = get_isf_espanhol_calendario();
